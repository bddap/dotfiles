...
def token(text, start, end):
    return SimpleNamespace(text=text, start_ts=start, end_ts=end)


def result(tokens, seconds):
    return SimpleNamespace(tokens=tokens, audio=np.zeros(int(seconds * tts_read.SAMPLE_RATE), np.float32))


class SentenceSpans(unittest.TestCase):
    def test_spans_cover_sentences_in_order(self):
...
class Collect(unittest.TestCase):
    def test_repeated_word_maps_to_successive_occurrences(self):
    ...
    def test_later_chunks_are_offset_by_earlier_audio(self):
        text = "one two"
    ...
    def test_unmatched_token_does_not_shift_later_words(self):
    ...
    def test_tokens_without_timestamps_are_skipped(self):
...
class Locate(unittest.TestCase):
    spans = [(0, 9), (10, 19)]
    chunks = [(np.zeros(2 * tts_read.SAMPLE_RATE, np.float32), [(0, 3, 0.0, 1.0), (4, 9, 1.0, 2.0)]),
              (np.zeros(1 * tts_read.SAMPLE_RATE, np.float32), [(0, 5, 0.0, 0.5)])]
    starts = {0: 0, 1: int(3e9)}

    def test_silence_gap_between_chunks_lights_nothing(self):
        self.assertEqual(tts_read.locate(int(2.5e9), 0, self.spans, self.starts, self.chunks), (1, None))

    def test_word_is_found_relative_to_its_chunk_start(self):
    ...
    def test_origin_skips_sentences_before_the_seek_point(self):
        self.assertEqual(tts_read.locate(int(0.25e9), 1, self.spans, {1: 0}, self.chunks), (1.25, (10, 15)))

    def test_empty_chunk_is_stepped_over(self):
    ...
    def test_unpushed_chunk_waits_and_end_is_past_last_chunk(self):
...
class Synthesis(unittest.TestCase):
    ...
    def test_word_timestamps_are_monotonic_and_cover_the_sentence(self):
